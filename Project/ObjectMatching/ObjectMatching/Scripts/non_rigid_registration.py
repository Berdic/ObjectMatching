## Non-rigid registration, based on  http://gmmreg.googlecode.com
...
def obj_L2_TPS(param, basis, kernel, scene, scale, _lambda):
    ...
    print("shape of basis.T", basis.T.shape)
    grad = dot(basis.T, grad)
    grad[d+1:n] += 2*_lambda*dot(kernel,tps_param)
    grad = grad.reshape(d*n)
    return energy, grad
...
def non_rigid_registration(model, scene):
...
def GaussTransform(MatrixA, MatrixB,  m, n, dim, scale):
    dist_ij, cross_term = 0, 0
    cost_ij = 0
    grad = np.zeros((m,dim), dtype = np.float)
    grad[:] = 0
    for i in range (0,m):
        for j in range(0,n):
            dist_ij = 0
            for d in range(0,dim):
                id = i*dim + d
                jd = j*dim + d
                dist_ij += np.square( MatrixA[i,d] - MatrixB[j,d])
                cost_ij = np.exp(-dist_ij/np.square(scale))

            for d in range(0,dim):
                id = i*dim + d
                jd = j*dim + d
                grad[i,d] += -cost_ij*2*(MatrixA[i,d] - MatrixB[j,d])
            cross_term += cost_ij   #/* printf("cross_term = %.3f\n", cross_term); */
    #for t in range(0, m*dim) :
    grad /= m * n * np.square(scale)
    #print(cross_term/(m*n), grad.shape)
    return cross_term /(m*n), grad

def gauss_transform(MatrixA, MatrixB, scale):
    m = MatrixA.shape[0]
    n = MatrixB.shape[0]
    if (len(MatrixA.shape)> 1):
        ...
        dim = 1
    return  GaussTransform(MatrixA, MatrixB, m, n, dim, scale)
    #print result
